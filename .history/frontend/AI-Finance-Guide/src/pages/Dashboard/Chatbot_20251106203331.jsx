  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/v1/financial-advice", {
        ...
        body: JSON.stringify({ userId: "USER_ID_HERE", messages }), // replace with actual userId if you have auth
      });
      ...
      <div className="h-[calc(100vh-140px)] flex flex-col">
        ...
        <div className="flex-1 grid grid-cols-1 xl:grid-cols-4 gap-6 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto min-h-0 space-y-4">
                {messages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.sender === "user" ? "bg-purple-600 text-white rounded-br-none shadow-lg" : "bg-gray-100 text-gray-800 rounded-bl-none border border-gray-200/50 shadow-sm"}`}>
                      <div className="flex items-start space-x-2">
                        {msg.sender === "bot" && (
                          <div className="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center text-xs text-white font-bold mt-1 flex-shrink-0">AI</div>
                        )}
                        <div className="flex-1">
                          <p className="whitespace-pre-wrap text-sm">{msg.text}</p>
                        </div>
                        {msg.sender === "user" && (
                          <div className="w-6 h-6 bg-gray-500 rounded-full flex items-center justify-center text-xs text-white font-bold mt-1 flex-shrink-0">You</div>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                {loading && (
                  <div className="flex justify-start">
                    <div className="bg-gray-100 text-gray-800 rounded-2xl rounded-bl-none px-4 py-3 border border-gray-200/50 shadow-sm flex items-center space-x-2">
                      <div className="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center text-xs text-white font-bold flex-shrink-0">AI</div>
          ...
          <div className="xl:col-span-1 flex flex-col min-h-0">
            <div className="flex flex-col bg-white rounded-2xl p-6 border border-slate-200 shadow-sm h-full min-h-0 mt-auto">
