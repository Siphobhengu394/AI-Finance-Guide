...
const Chatbot = () => {
  ...
  const [advice, setAdvice] = useState(null); // AI financial advice JSON
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Financial advice API (Llama 3 via Groq Cloud)
      const adviceRes = await fetch("http://localhost:5000/api/financial-advice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: "123" }) // replace with actual user info
      });
            ...
            <div className="flex flex-col bg-white rounded-2xl p-4 shadow-sm h-full min-h-0">
              ...
              {advice && (
                <div className="mt-4 bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                  <h3 className="font-semibold text-gray-800 text-lg mb-2">📊 AI Financial Insights</h3>
                  <p className="text-sm text-gray-700 mb-2"><strong>Summary:</strong> {advice.summary}</p>
                  {advice.insights && advice.insights.length > 0 && (
                    <div className="mb-2">
                      <strong>Insights:</strong>
                      <ul className="list-disc ml-5 text-sm text-gray-700">
                        {advice.insights.map((insight, i) => <li key={i}>{insight}</li>)}
                      </ul>
                    </div>
                  )}
