...
const Chatbot = () => {
  const currentUser = useUserAuth(); // Assuming it returns { id, name, ... }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let botReply = data.advice.summary || "Sorry, I couldn't process your request.";

      if (data.advice.insights && data.advice.insights.length) {
        botReply += "\n\nInsights:\n" + data.advice.insights.join("\n");
      }
      if (data.advice.recommendations && data.advice.recommendations.length) {
        botReply += "\n\nRecommendations:\n" + data.advice.recommendations.join("\n");
      }
